...
const {
	extractName,
	compareAndSaveChatIds,
	removeChatId
} = require('./chatIdManager');
	...
	bot.onText(/logs/i, async (msg) => {
		if (msg.chat.id.toString() === admin) {
			...
			await bot.sendMessage(
				msg.chat.id,
				'You are not authorized to view the logs.'
			);
		}
	});
}
...
async function botOnCallback(bot, chatIdsPath, college, savedNoticesPath) {
	bot.on('callback_query', async (query) => {
		...
		await log(
			`${name} selected: ${queryData === 'latest' ? 'Latest Notices' : 'No'}`
		);
	});
}
...
async function handleLatestNotices(
	bot,
	chatId,
	college,
	savedNoticesPath,
